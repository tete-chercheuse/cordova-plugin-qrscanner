...
public class QRScanner extends CordovaPlugin implements BarcodeCallback
{
	...
	private static class QRScannerError
	{
		...
		try
		{
			switch (action)
			{
				...
				case "scan":
					...
					return true;
				case "cancelScan":
					cordova.getThreadPool().execute(() -> cancelScan(callbackContext));
					return true;
				case "openSettings":
	...
	private void doswitchFlash(final boolean toggleLight, final CallbackContext callbackContext)
	...
	{
		this.cordova.getActivity().runOnUiThread(() -> webView.getView().setBackgroundColor(Color.parseColor("#EEEEEE")));
		showing = false;
	}
	...
	private void getStatus(CallbackContext callbackContext)
	{
		...
		status.put("canOpenSettings", boolToNumberString(true));
